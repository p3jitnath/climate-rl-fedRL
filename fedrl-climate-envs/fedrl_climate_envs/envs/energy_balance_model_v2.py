...
EBM_LATITUDES = 96
NUM_EBM = 2
EBM_SUBLATITUDES = EBM_LATITUDES // NUM_EBM
...
class EnergyBalanceModelEnv(gym.Env):
    ...
    def __init__(self, seed=None, render_mode=None):
        ...
        self.utils = Utils()
        self.seed = seed
        self.flwr_client = self.cid = int(os.getenv("FLWR_CLIENT"))

        print(f"[RL Env] Environment ID: {self.flwr_client}", flush=True)
        ...
        self.redis.put_tensor(
            f"SIGALIVE_S{self.flwr_client}", np.array([1], dtype=np.int32)
        )
